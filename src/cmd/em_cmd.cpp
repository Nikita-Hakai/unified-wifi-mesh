...
void em_cmd_t::init()
{
    switch (m_type) {
        ...
        case em_cmd_type_assoc_sta_link_metrics_query:
            strncpy(m_name, "link_metrics", strlen("link_metrics") + 1);
            m_svc = em_service_type_agent;
            break;
...
#define BUS_EVENT_TYPE_2S(x) case x: return #x;
    ...
    BUS_EVENT_TYPE_2S(em_bus_event_type_assoc_sta_link_metrics_query)
    }
}   
...
em_bus_event_type_t em_cmd_t::cmd_2_bus_event_type(em_cmd_type_t ctype)
{
    ...
    switch (ctype) {
        ...
        case em_cmd_type_assoc_sta_link_metrics_query:
            type = em_bus_event_type_assoc_sta_link_metrics_query;
            break;
    }
