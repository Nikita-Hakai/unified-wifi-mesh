            ...
            for (k = 0; k < cJSON_GetArraySize(bss_list_obj); k++) {
                ...
                for (l = 0; l < cJSON_GetArraySize(sta_list_obj); l++) {
                    ...
                    if (cJSON_GetObjectItem(request_mode_obj, "Steering_Opportunity") != NULL) {
                        ...
                        steer_param.params[steer_param.num].request_mode = static_cast<unsigned int> (cJSON_GetNumberValue(steer_opp));
                    }
                    // Check for "Steering_Mandate"
                        ...
                        steer_param.params[steer_param.num].request_mode = static_cast<unsigned int> (cJSON_GetNumberValue(steer_mandate));
                    }
                    ...
                    steer_param.params[steer_param.num].steer_opportunity_win = static_cast<unsigned int> (cJSON_GetNumberValue(opportunity_obj));
                    timer_obj = cJSON_GetObjectItem(steer_obj, "BTMDisassociationTimer");
                    steer_param.params[steer_param.num].btm_disassociation_timer = static_cast<unsigned int> (cJSON_GetNumberValue(timer_obj));
                    op_class_obj = cJSON_GetObjectItem(steer_obj, "TargetBSSOperatingClass");
                    steer_param.params[steer_param.num].target_op_class = static_cast<unsigned int> (cJSON_GetNumberValue(op_class_obj));
                    channel_obj = cJSON_GetObjectItem(steer_obj, "TargetBSSChannel");
                    steer_param.params[steer_param.num].target_channel = static_cast<unsigned int> (cJSON_GetNumberValue(channel_obj));
